"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var http_1 = require("@angular/http");
require("rxjs/add/operator/map");
require("rxjs/add/operator/toPromise");
var DiscussionService = (function () {
    function DiscussionService(http) {
        this.http = http;
        this.discussionsUrl = 'api/discussions';
    }
    DiscussionService.prototype.getDiscussions = function () {
        return this.http.get(this.discussionsUrl)
            .toPromise()
            .then(function (response) { return response.json(); })
            .catch(this.handleError);
    };
    DiscussionService.prototype.getDiscussion = function (id) {
        return this.getDiscussions()
            .then(function (discussions) { return discussions.find(function (discussion) { return discussion.id === id; }); });
    };
    DiscussionService.prototype.createDiscussion = function (discussion) {
        var body = JSON.stringify(discussion);
        var headers = new http_1.Headers({ 'Content-Type': 'application/json' });
        var options = new http_1.RequestOptions({ headers: headers });
        return this.http.post(this.discussionsUrl, discussion, options)
            .map(function (res) { return res.json().data; });
    };
    DiscussionService.prototype.handleError = function (error) {
        console.error('An error occurred', error);
        return Promise.reject(error.message || error);
    };
    return DiscussionService;
}());
DiscussionService = __decorate([
    core_1.Injectable(),
    __metadata("design:paramtypes", [http_1.Http])
], DiscussionService);
exports.DiscussionService = DiscussionService;
//# sourceMappingURL=discussion.service.js.map